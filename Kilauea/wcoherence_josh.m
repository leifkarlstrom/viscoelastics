function [wcoh,wcs,P,coi,wtx,wty] = wcoherence_josh(x,y,varargin)
%Wavelet coherence
...
if IsMATLAB
    ...
    [Ts,fs,FLimits,PLimits,nv,nos,~,mincoherence,tsFlag,fsFlag,...
        normalizedfreq,~,skipreg] = parseinputsMATLAB_josh(numel(tempx), varargin{:});
else
...
function [ts,fs,FLimits,PLimits,nv,...
    ...
    engunits,skipreg] = parseinputsMATLAB_josh(N,varargin)
...
FLimits = validateinputs(N,FLimits,PLimits,nv,mincoherence,nos,numoct,...
    fs,ts,tsFlag,fsFlag,PL_flg,FL_flg,errFlag,skipreg);
end
...
function FLimits = validateinputs(N,FLimits,PLimits,nv,mincoherence,...
    nos,numoct,Frequency,Period,tsFlag,fsFlag,PL_flg,FL_flg,errFlag,skipreg)
...
validateattributes(skipreg,{'numeric'},{'finite'},...
    'wcoherence','skipreg');
...
function plotcoherenceperiod(wcoh,wcs,period,t,coitmp,nov,mc,plotstring,skipreg)
...
%
if ~isempty(skipreg)
    wcoh(:,t>skipreg(1)&t<skipreg(2)) = 0;
end
...
arrowpatch = 0.5*[-1 0 0 1 0 0 -1; 0.1 0.1 0.5 0 -0.5 -0.1 -0.1]';
...
for ii=numel(tgrid):-1:1
    ...
    patch(tgrid(ii)+rotarrow(:,1)*dx,pgrid(ii)+rotarrow(:,2)*dy,[1 0 0],...
